...
#Coloca los nombre de los ejes
	set xlabel "$\\text{Concentración en la solución/mM}$"	font ",14" textcolor rgb "gray40" offset  0,-2
	set ylabel "Concentracion en la película / M" font ",14" textcolor rgb "gray40" offset -2,0
...
#FITEO
	f(x) = x/(x+a)+b
	fit f(x) "Ru Lagmuir.txt" using ($1):($2) via a,b
	
	#f(x) = x/(c*x+a)+b
	#fit f(x) "Ru Lagmuir.txt" using ($1):($2) via a,b,c
	stat "Ru Lagmuir.txt" using 1:2

	g(x) = x/(x+d)+e
	fit g(x) "Ru Lagmuir.txt" using ($1):($3) via d,e
	
	#g(x) = x/(f*x+d)+e
	#fit g(x) "Ru Lagmuir.txt" using ($1):($3) via d,e,f
	stat "Ru Lagmuir.txt" using 1:3
...
plot "Ru Lagmuir.txt" using ($1):($2) with points ls 1,f(x) with lines ls 4, "Ru Lagmuir.txt" using ($1):($3) with points ls 2,g(x) with lines ls 3
