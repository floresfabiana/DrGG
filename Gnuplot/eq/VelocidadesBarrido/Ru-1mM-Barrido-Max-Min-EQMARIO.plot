...
#FITEO
	 ...
	 set label 8 sprintf("R=%3.4f",STATS_correlation) at 4,2.35 font ",6" textcolor rgb "gray40"
	  ...
	  fit g(x) "Ru-1mM-Barrido-Max-Min-EQMARIO.txt" using (log10($1)):(log10($5)) every ::0::4 via c,d
...
plot "Ru-1mM-Barrido-Max-Min-EQMARIO.txt" using (log10($1)):(log10($3)) title "Corriente \'anodica" ls 1, [1.77:2.15] f(x) with lines ls 1 notitle, [1.9:4.9] i(x) with lines ls 1 notitle, \
"Ru-1mM-Barrido-Max-Min-EQMARIO.txt" using (log10($1)):(log10($5)) title "Corriente cat\'odica" ls 2, [1.6:2.13] g(x) with lines ls 2 notitle, [1.9:4.9]h(x) with lines ls 2 notitle\
