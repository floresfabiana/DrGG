...
set terminal tikz
set output "GAMRY-Ru1mM-Max-Min.tikz"
...
#FITEO
	 f(x) = a*x + b
	 fit f(x) "GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($3)) via a,b
	 stat "GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($3))
	 ...
	 g(x) = c*x + d
	 fit g(x) "GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($5)) via c,d
	 stat "GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($5))
...
plot "GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($3)) with points ls 1 title "Corriente \'anodica", f(x) with lines ls 1 notitle, \
"GAMRY-Ru1mM-Max-Min.txt" using (log10($1)):(log10($5)) with points ls 2 title "Corriente cat\'odica", g(x) with lines ls 2 notitle #h(x) with lines ls 2 notitle