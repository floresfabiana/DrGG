...
\subsection{Funcionalización}
	...
	Para obtener resultados sólidos sobre fenómenos de transporte dentro de las películas, que lleven a interpretaciones verosímiles, se necesitan películas donde la estabilidad estructural no esté comprometida. En las secciones precedentes se ha demostrado que la incorporación de un 10\% de Zr a las películas de SiO$_2$, utilizando el método de síntesis por alto vacío, mejora la estabilidad mecánica y la resistencia química sin perder poder de adsorción, incluso frente \pdmF\space calcinadas. Es por ellos que todos los experimentos que se presentan en esta sección fueron realizados con dichos sistemas.

	Las moléculas incorporadas fueron dihexadecilfosfato (DHDP) y 3-amino\- propil trietoxisilano (APTES). Dicha elección se basó en trabajos previos donde estudian en detalle la incorporación de estás moléculas, ya sea en películas delgadas mesoporosas de SiO$_2$, TiO$2$, ZrO$_2$ o mixtas.\cite{Calvo2009b,Angelome2008,Fattakhova-Rohlfing2007,Andrieu-brunsen2014,Andrieu-Brunsen2015} Hasta ahora no se han reportado en la literatura trabajos donde se incorporen estas funcionalizaciones sobre electrodos recubiertos con películas delgadas mesoporosas mixtas Zr$_x$Si$_{1-x}$O$_2$. Además, los resultados expuestos en esta sección cuenta con el plus de que fueron obtenidos en un dispositivo micromaquinado, es decir, que las reacciones de funcionalización se llevaron en electrodos diferentes pero en un único sensor. El capítulo que sigue ahonda en los detalle sobre la fabricación de los sensores.
	
		\subsubsection{Incorporación del DHDP}
	
			La funcionalización con DHDP se llevó a cabo adaptando la reacción utilizada por Angelomé\cite{Angelome2008}. Se utilizó \SI{3}{\milli\Molar} de DHDP sobre películas delgadas de Si$_x$Zr$_{1-x}$O$_2$ (\pdmZ$^P_3$). Las condiciones experimentales se pueden consulta en la sección \ref{sub:funcionalizaci_n_de_las_pdm}, pág. \ref{sub:funcionalizaci_n_de_las_pdm}. En dicho trabajo incorpora DHDP en películas mixtas de Si$_x$Zr$_{1-x}$O$_2$ y Si$_x$Ti$_{1-x}$O$_2$. Se concluye allí que la reacción se trata de una unión débil de complejación superficial entre el fosfonato y el centro metálico, en este caso el Zr. El esquema de la figura \ref{esq:dhdp-esquema} representa una \pdmZ$^P_3$ luego de ser funcionalizada.
		    ...
		    Una vez realizada la modificación superficial de las películas se llevaron a cabo sobre las mismas mediciones EQ con \fe\space y \ru. El resultado de la mismas son los voltagramas que se presentan en las figuras \ref{fig:dhdp-vc-fe} y \ref{}. En el caso del \fe\space se ve que luego de llevar a cabo la funcionalización con DHDP, la sonda negativa logra permear hasta la superficie del electrodo, mientras que en las \pdmZ\space sin funcionalizar dicha sonda quedaba totalmente excluida (figura \ref{}).
		    ...
		    El caso del \ru\space 
		    	...
		    	\begin{figure}[ht]	
					\centering
			 	    \includegraphics[width=0.70\textwidth]{Graficos/dhdp-Ru.pdf}
			        \caption[Voltagrama para \pdmF$^P_3$ con \rucompletos]{voltagramas donde se compara la respuesta .....90 cicloas para cada uno.... etc}
			...
			Los resultados de ambos experimentos se se pueden interpretar en base a dos fenómenos que apuntan en la misma dirección. Uno es la disminución de la carga superficial al incorporar el DHDP, esto permite que la exclusión del \fe\space no sea total y que el \ru\space se sienta menos atracción eletrostatica por las paredes del mesoporoso. Y el otro, posiblemente de mayor peso, es la capa hidrofóbica que se forma dentro de los poros. Dicha capa dificulta la adsorción de \ru dentro de  \pdmZ$^P_3$ a la vez que favorece el traslado de \ru\space libre al electrodos. Esto se puede observar en el voltagrama de la figura.... donde se compara la señal para \ru de una pelicula fucnionalizada con DHDP y otra que no fue funcionalizada. Alli se oibserva que la cinetica de absorcion en muy distinta. Mientas que en las peliuclas sin funcionalizar la señan crece paulatinamete .... en la fuincnioalizada crece rapido. 

		\subsubsection{Incorporación del APTES}
