...
\section{Caso de estudio: \texorpdfstring{\aminorutenioCompleto}{Ru(NH3)CL3}}
	...
	\subsection{Mecanismo de transporte de carga}
	 ...
	 Si se considera que los nanoporos son esféricos, monodispersos y distribuidos uniformemente en la película (consultar capítulo \ref{chap:Mesoporosos}), se puede calcular la cantidad \ru\space por poro. Para ello se multiplica la concentración de \ru\space adsorbido ($C$) por el volumen de poro ($V_{poro}$ y el número de Avogadro ($N_{A}$) Y, a su vez, como se encuentran adsorbidos, se puede dividir por la superficie del poro ($S_{poro}$) de forma de obtener el numero de sitios redox por unidad de volumen, o, tomando la raiz cuadrada de la inversa, la distancia promedio entre dos sitios redox ($d_H$), según la ecuación \ref{eq:distancia_redox}. 
	 	\begin{equation}
					d_H=2\sqrt{\frac{S_{poro}}{\pi\, V_{poro}\, N_A\, C}}
					\label{eq:distancia_redox}
			\end{equation}
	 Teniendo en cuenta una concentración de saturación de \SI{1}{\Molar} en una película de \SI{200}{nm}, la distancia entre sitios redox es de $d_H=$\SI{1.25}{nm}. Este valor es compatible con el modelo de \textit{eletron hopping} propuestos y con la concentración calculada. Un valor de $d_H$ muy grande seria un indicador de que, o bien la película no esta saturada o bien los sitios redox están muy lejos para permitir el salto entre sitios, mientas que un valor de $d_H$ muy pequeño, seria absurdo, ya que se estarían solapando los sitios redox (para $d_H < \phi^{e}$). 
	 	\begin{figure}[ht]
					\centering
			 	    \includegraphics[width=0.70\textwidth]{Graficos/langmuir.pdf}
			        \caption[Isoterma de Langmuir]{Isoterma de Langmuir a T=\SI{25}{\celsius}donde se gráfica la concentración de \aminorutenio\space adsorbido en función de la concentración en solución colocada inicialmente.}
	 ...
	 En el esquema de la figura \ref{fig:sitios_redox} se ejemplifica el mecanismo propuesto. Se puede entonces, calcular el coeficiente de difusión $D_H$ para la transferencia de electrones. Se abarcó el calculo de este parámetro desde dos enfoques diferentes, uno utilizando la técnica de voltametría de corriente alterna (VCA) y otra utilizando voltametrías cíclicas (CV) a distintas velocidades de barrido. Los detalles experimentales para ambas técnicas fueron explicados en la sección \ref{sec:medidas_eq}, pág. \pageref{sec:medidas_eq}.
	 		\begin{figure}[ht]
					\centering
			 	    \includegraphics[width=0.70\textwidth]{Esquemas/sitios_redox.pdf}
			        \caption[Mecanismo de transferencia de electrones]{Pictograma .....Mecanismo por el cual.........}
			        \label{fig:sitios_redox}
			      	\end{figure} 
 
	 \subsubsection*{Calculo de DH mediante VCA}	
	 
	 Tomando como base el trabajo de Tagliazucchi y col.\cite{Tagliazucchi2010a} (donde calcula el coeficiente de difusión para un complejo de osmio adsorbido en un polímero), se tomaron VC en de una película mesoporosa saturada con \ru\space a distintas velocidades de barrido, desde \SI{50}{\milli\volt\per\second} a \SI{50}{\volt\per\second}. Luego se graficó, el desplazamiento del potencial de pico por un lado y la intensidad de pico por otro, ambas variables en función de la velocidades de barrido (gráficos \ref{} y \ref{} respectivamente).

	 
	 \subsubsection*{Calculo de DH mediante VC}
