...
\section{Caso de estudio: ferroceno metanol}\label{sec:difusion}
	...
	\subsection{En sistemas mesoporosos calcinados}
	 ...
	 Se colocó una solución de \fc\space \SI{1}{\milli\Molar} y se realizaron una serie de voltametrías a diferentes velocidades de barrido, con el propósito de estimar el coeficiente de difusión\index{difusión}. En la figura \ref{fig:difusion_calcinado}, donde se gráfico la intensidad de pico (i$_p$) en función de la velocidad de barrido\index{velocidad!de barrido} ($\nu$), se observa que $\text{i}_p \propto \nu^{1/2}$, lo que indica que se trata de un proceso controlado por difusión\index{difusión}. Ahora bien, esta difusión\index{difusión} tiene dos contribuciones, una de la sonda\index{sonda} en solución y otra de la sonda\index{sonda} dentro de los poros. Por lo tanto se calcula un coeficiente aparente que contempla ambas contribuciones. Se puede, en principio aplicar el modelo de Randles-Sevcik\index{Randles-Sevcik} (ec. \ref{eq:dapp_bajaT}) en el cual se supone un coeficiente de difusión\index{difusión} independiente del potencial, que la sonda\index{sonda} no interactúa con la matriz y que la intensidad de corriente solo depende de la concentración de la sonda\index{sonda}, de la difusión\index{difusión} y de la velocidad de barrido\index{velocidad!de barrido}. En base a este modelo se calculó el coeficiente de difusión\index{difusión} aparente ($D_{ap}$) arrojando un valor de $D_{ap}$=\SI{6,1e-6}{\square\cm\per\second}, el cual es según la bibliografía un típico valor para moléculas en soluciones acuosas. \cite{koryta1993,Otal2006}
	...
	\subsection{En sistemas mesoporoso sintetizados a baja temperatura}
		...
		Para una película de \SI{200}{nm} $D$ resultó ser de \SI{2.5e-9}{\square\cm\per\second}, un valor esperable ya que la difusión\index{difusión} se encuentra muy impedida en estos sistemas más <<cerrados>>. El orden de magnitud de éste coeficiente es comparable con los reportados para las permeación\index{permeacion} de sonda\index{sonda}s a través de películas poliméricas.\cite{Kolb1993}
...
\section{Caso de estudio: \texorpdfstring{\aminorutenioCompleto}{Ru(NH\index{amoniaco}3)CL3}}
	...
	\subsection{Mecanismo de transporte\index{transporte} de carga}
	 ...
	 \subsubsection*{Calculo de $D_e$ mediante VC}	
	     ...
	     La figura \ref{fig:corrimiento-potenciales} indica mediante flechas rojas la velocidad característica donde, aparentemente, la transferencia de carga es mas rápido que el transporte\index{transporte} de carga, sin embargo el procesos de transferencia de carga enmascara la separación de picos y, por lo tanto, sería impreciso establecer un valor de  $\nu_{\scriptscriptstyle{D}}$ de este gráfico.
